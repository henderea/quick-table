...
declare type ColumnDef = { cellType?: string, data?: string, render?: (data: any | null, row: Object) => string, html?: boolean, cssClass?: string };
...
function columnId(column: number | ColumnId): ColumnId {
    if (column instanceof ColumnId) { return column; }
    if (!(columnIds[column] instanceof ColumnId)) {
        columnIds[column] = new ColumnId(column);
    }
    return columnIds[column];
}

class CellId {
    constructor(readonly rowId: RowId, readonly columnId: ColumnId) { }
...
class RowId {
    ...
    constructor(readonly rowIndex: number, readonly isHead: boolean) { }

    [_getCellId](columnId: ColumnId): CellId {
        if (!(this.cellIds[String(columnId)] instanceof CellId)) {
            this.cellIds[String(columnId)] = new CellId(this, columnId);
        }
...
function rowId(row: number | RowId, isHead: boolean = false): RowId {
    ...
    if (!(rowIds[isHead ? 'head' : 'body'][row] instanceof RowId)) {
        rowIds[isHead ? 'head' : 'body'][row] = new RowId(row, isHead);
    }
    return rowIds[isHead ? 'head' : 'body'][row];
}

function cellId(row: number | RowId | CellId, column: number | ColumnId = 0, isHead = false): CellId {
    if (row instanceof CellId) { return row; }
    return rowId(row, isHead)[_getCellId](columnId(column));
}

export default ($: JQuery, _: LoDashStatic) => {
    class EventEmitter {
    ...
    class Cell {
        constructor(readonly quickTable: QuickTable, readonly cellId: CellId) { }
    ...
    class Cells extends QTIterable<Cell> {
        readonly cellIds: CellId[];
        constructor(readonly quickTable: QuickTable, cellIds: (CellId | Cell | Cells)[]) {
            super();
    ...
    class Column extends EventEmitter {
        constructor(readonly quickTable: QuickTable, readonly columnId: ColumnId) {
            super();
        ...
        get headerCells(): Cells { return new Cells(this.quickTable, this.headerCellIds); }

        get bodyCells(): Cells { return new Cells(this.quickTable, this.bodyCellIds); }

        get cells(): Cells { return new Cells(this.quickTable, this.cellIds); }
    }

    class Columns extends QTIterable<Column> {
        constructor(readonly quickTable: QuickTable, readonly columnIds: ColumnId[]) {
            super();
        ...
        rowCells(row: number | RowId, isHead: boolean = false): Cells { return new Cells(this.quickTable, _.map(this.columns, c => c.cellId(row, isHead))); }
    ...
    class Row {
        ...
        constructor(readonly quickTable: QuickTable, readonly rowId: RowId) { }
        ...
        cell(column: number | ColumnId): Cell | null {
            ...
            if (!(this._cells[String(column)] instanceof Cell)) {
                this._cells[String(column)] = new Cell(this.quickTable, this.cellId(column));
            }
    ...
    class Rows extends QTIterable<Row> {
        constructor(readonly quickTable: QuickTable, readonly rowIds: RowId[]) {
            super();
        ...
        columnCells(column: number | ColumnId): Cells { return new Cells(this.quickTable, this.columnCellIds(column)); }
    ...
    class QuickTable extends EventEmitter {
        ...
        constructor(table: JQuery, initFunc: ((table: QuickTable) => void) | null = null) {
            super();
            this._table = $(table);
        ...
        get columns(): Columns { return new Columns(this, this.columnIds); }
        ...
        column(column: number | ColumnId): Column | null {
            ...
            if (!(this._columns[String(column)] instanceof Column)) {
                this._columns[String(column)] = new Column(this, column);
            }
        ...
        getColumns(...columns: (number | ColumnId)[]): Columns { return new Columns(this, _.map(_.flatten(columns), c => this.columnId(c))); }
        ...
        getAllRows(isHead: boolean): Rows { return new Rows(this, this.getAllRowIds(isHead)); }
        ...
        getBodyRows(...rows: (number | RowId)[]): Rows { return new Rows(this, _.map(_.flatten(rows), r => this.rowId(r, false))); }

        getHeaderRows(...rows: (number | RowId)[]): Rows { return new Rows(this, _.map(_.flatten(rows), r => this.rowId(r, true))); }

        getRows(...rowIds: RowId[]): Rows { return new Rows(this, _.filter(_.flatten(rowIds), r => r instanceof RowId)); }
        ...
        row(row: number | RowId, isHead: boolean = false): Row | null {
            ...
            if (!(this._rows[String(row)] instanceof Row)) {
                this._rows[String(row)] = new Row(this, row);
            }
    ...
    class QuickTables extends QTIterable<QuickTable> {
        private _tables: QuickTable[] = [];
        constructor(tables: QuickTable[] = []) {
            super();
    ...
    $.fn.QuickTable = function (initFunc: ((table: QuickTable) => void) | null = null): QuickTable | QuickTables {
        let tables: QuickTables = new QuickTables();
